...
#ifdef ARROW_WITH_OPENTELEMETRY_JAEGER
...
#endif
    } else if (env_var == "jaeger") {
#ifdef ARROW_WITH_OPENTELEMETRY_JAEGER
      namespace jaeger = otel::exporter::jaeger;

      constexpr char kJaegerHostEnvVar[] = "ARROW_TRACING_JAEGER_HOST";
      constexpr char kJaegerPortEnvVar[] = "ARROW_TRACING_JAEGER_PORT";
      jaeger::JaegerExporterOptions exporter_options;
      exporter_options.server_addr = "localhost";
      exporter_options.server_port = 6831;
      exporter_options.transport_format = jaeger::TransportFormat::kThriftUdpCompact;

      auto maybe_host = arrow::internal::GetEnvVar(kJaegerHostEnvVar);
      if (maybe_host.ok()) {
        auto host = maybe_host.MoveValueUnsafe();
        if (!host.empty()) exporter_options.server_addr = std::move(host);
      }
      auto maybe_port = arrow::internal::GetEnvVar(kJaegerPortEnvVar);
      if (maybe_port.ok()) {
        auto port = maybe_port.MoveValueUnsafe();
        if (!port.empty())
          exporter_options.server_addr = static_cast<uint16_t>(std::stoi(port));
      }

      return std::unique_ptr<sdktrace::SpanExporter>(
          new jaeger::JaegerExporter(exporter_options));
#else
      ARROW_LOG(WARNING) << "Requested " << kTracingBackendEnvVar
                         << "=jaeger but Arrow was built without ARROW_THRIFT";
#endif
