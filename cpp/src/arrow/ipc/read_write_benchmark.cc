...
static void ReadFileAsync(benchmark::State& state) {  // NOLINT non-const reference
  // 1MB
  constexpr int64_t kTotalSize = 1 << 20;
  auto options = ipc::IpcWriteOptions::Defaults();
...
static void ReadCompressedFileAsync(
...
BENCHMARK(ReadFileAsync)->RangeMultiplier(4)->Range(1, 1 << 13)->UseRealTime();
...
BENCHMARK(ReadCompressedFileAsync)->RangeMultiplier(4)->Range(1, 1 << 10)->UseRealTime();
