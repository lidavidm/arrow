 ...
 public:
  ...
  Status DoneWriting() {
    ...
    if (!this->stream()->WritesDone()) {
      ...
      return Finish(Status::IOError("Could not flush pending record batches"));
    }
    return Status::OK();
  }
 ...
 public:
  ...
  Status EnsureDataStarted() {
    if (!batch_reader_) {
      ...
      if (!skipped_to_data) {
        return OverrideWithServerError(
            Status::IOError("Server never sent a data message"));
      }
...
class GrpcStreamWriter : public FlightStreamWriter {
 public:
  ...
  Status Begin(const std::shared_ptr<Schema>& schema) override {
    ...
    // See Close().
    return ipc::internal::OpenRecordBatchWriter(std::move(payload_writer), schema,
                                                &batch_writer_);
  }
  ...
  Status WriteMetadata(std::shared_ptr<Buffer> app_metadata) override {
    ...
    if (!internal::WritePayload(payload, writer_->stream().get())) {
      return writer_->Finish(Status::IOError("Could not write metadata to stream"));
    }
    return Status::OK();
  }
...
class DoPutPayloadWriter : public ipc::internal::IpcPayloadWriter {
 public:
  ...
  Status WritePayload(const ipc::internal::IpcPayload& ipc_payload) override {
    ...
    if (!internal::WritePayload(payload, writer_->stream().get())) {
      return writer_->Finish(Status::IOError("Could not write record batch to stream"));
    }
    return Status::OK();
  }
...
Status GrpcStreamWriter<ProtoReadT, FlightReadT>::Open(
  ...
  if (schema) {
    ...
    if (!internal::WritePayload(payload, instance->writer_->stream().get())) {
      return writer->Finish(Status::IOError("Could not write descriptor to stream"));
    }
  }
...
Status FlightClient::DoExchange(const FlightCallOptions& options,
                                ...
                                std::unique_ptr<FlightStreamReader>* reader,
                                std::unique_ptr<FlightStreamWriter>* writer) {
  return impl_->DoExchange(options, descriptor, reader, writer);
}
