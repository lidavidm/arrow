 ...
 private:
  ...
  Status EnsureDataStarted() {
    if (!batch_reader_) {
      ...
      std::unique_ptr<FlightIpcMessageReader<GrpcStream>> message_reader(
...
class GrpcMessageWriter : public FlightMessageWriter {
 ...
 private:
  ...
  // TODO: this class should share with RecordBatchStreamImpl
...
class FlightServiceImpl : public FlightService::Service {
 public:
  ...
  grpc::Status DoExchange(
    ...
    SERVICE_RETURN_NOT_OK(flight_context, message_reader->Init());
    auto writer = std::unique_ptr<GrpcMessageWriter>(new GrpcMessageWriter(stream));
    RETURN_WITH_MIDDLEWARE(flight_context,
