...
class IntegrationTestScenario : public arrow::flight::Scenario {
 public:
  ...
  arrow::Status RunClient(const arrow::flight::Location& location,
                          std::unique_ptr<arrow::flight::FlightClient> client) override {
    ...
    ABORT_NOT_OK(
        UploadReaderToFlight(table_reader.get(), *write_stream, *metadata_reader));
    ...
    for (const arrow::flight::FlightEndpoint& endpoint : info->endpoints()) {
      ...
      for (const auto location : locations) {
        ...
        if (!original_data->Equals(*retrieved_data)) {
          ...
          return arrow::Status::Invalid("Data does not match!");
        }
      }
    }
    return arrow::Status::OK();
...
int main(int argc, char** argv) {
  ...
  if (!FLAGS_scenario.empty()) {
    ...
    scenario = std::make_shared<IntegrationTestScenario>();
  }
  ...
  ABORT_NOT_OK(scenario->RunClient(location, std::move(client)));
