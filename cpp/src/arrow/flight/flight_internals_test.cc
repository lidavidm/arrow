...
TEST(FlightTypes, FlightDescriptor) {
  std::vector<FlightDescriptor> values = {
      ...
      FlightDescriptor::Command("select foo from table", /*accept_partial=*/true),
      FlightDescriptor::Path({}),
      FlightDescriptor::Path({"foo", "baz"}),
      FlightDescriptor::Path({"foo", "baz"}, /*accept_partial=*/true),
  };
  std::vector<std::string> reprs = {
      ...
      "FlightDescriptor<cmd = 'select foo from table', accept_partial>",
      "FlightDescriptor<path = ''>",
      "FlightDescriptor<path = 'foo/baz'>",
      "FlightDescriptor<path = 'foo/baz', accept_partial>",
  };
...
TEST(FlightTypes, FlightEndpoint) {
  ...
  std::vector<std::string> reprs = {
      "FlightEndpoint<ticket = Ticket<ticket = ''>, locations = {}, expiration_nanos = "
      "(nullopt)>",
      "FlightEndpoint<ticket = Ticket<ticket = 'foo'>, locations = {}, expiration_nanos "
      "= (nullopt)>",
      "FlightEndpoint<ticket = Ticket<ticket = 'bar'>, locations = {}, expiration_nanos "
      "= (nullopt)>",
      "FlightEndpoint<ticket = Ticket<ticket = 'foo'>, locations = "
      "{grpc+tcp://localhost:1024}, expiration_nanos = (nullopt)>",
      "FlightEndpoint<ticket = Ticket<ticket = 'bar'>, locations = "
      "{grpc+tcp://localhost:1024}, expiration_nanos = (nullopt)>",
      "FlightEndpoint<ticket = Ticket<ticket = 'foo'>, locations = "
      "{grpc+tls://localhost:1024}, expiration_nanos = (nullopt)>",
      "FlightEndpoint<ticket = Ticket<ticket = 'foo'>, locations = "
      "{grpc+tls://localhost:1024}, expiration_nanos = 42>",
      "FlightEndpoint<ticket = Ticket<ticket = 'foo'>, locations = "
      "{grpc+tcp://localhost:1024, grpc+tls://localhost:1024}, expiration_nanos = "
      "(nullopt)>",
  };
...
TEST(FlightTypes, FlightInfo) {
  ...
  std::vector<std::string> reprs = {
      ("FlightInfo<schema = (serialized), descriptor = FlightDescriptor<cmd = 'foo'>, "
       ...
       "expiration_nanos = (nullopt)>}, total_records = -1, total_bytes = 42, "
       "endpoints_ordered = false, retry_info = (nullopt)>"),
  };
