...
class UcxClientDataStream : public internal::ClientDataStream {
 public:
  ...
  Status Finish() {
    if (finished_) return MergedStatus();
...
class ARROW_FLIGHT_EXPORT UcxClientImpl
 ...
 public:
  ...
  Status Init(const FlightClientOptions& options, const Location& location,
    ...
    driver_.reset(new UcpCallDriver(ucp_context_, ucp_worker_, remote_endpoint_));
  ...
  Status Close() override {
    ...
    RETURN_NOT_OK(driver_->SendFrame(FrameType::kDisconnect, nullptr, 0));
